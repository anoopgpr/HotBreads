...
var json_fb='https://taghash.co/mloyalApps/facebookhotbreads.php';
var RSS_twitter='https://taghash.co/mloyalApps/twitter.php?uname=hotbreadshome';
var RSS_pinterest='http://103.25.128.61/projects/taghash/mloyalApps/pinteresttantra.php?brand=hotbreads';
var RSS_instagram='https://www.instagram.com/hotbreads/?__a=1';
var RSS_youtube='http://xceedfbapps.com/mloyalfeeds/youtube.php?brandname=hotbreads';
var RSS_tumblr='http://xceedfbapps.com/mloyalfeeds/tumblr.php?brandurl=http://hotbreads.tumblr.com';

var brandname='hotbreads';
var aboutus_det='hotbreads';
var brand_url='https://www.hotbreads.co.in/';
var cart = { cartdata: []};
...
var SERVER = "https://hotbreads.mloyalretail.com/m/";
var SERVER1 = "https://hotbreads.mloyalretail.com/";
...
$(document).ready(function(event){
			   ...
			   if($.mobile.activePage.is('#homepage')){
                 ...
                 //getContacts();
                  if(localStorage.getItem("usernamehotbreads")!=null && localStorage.getItem("usernamehotbreads")!='' && localStorage.getItem("passwordhotbreads")!=null && localStorage.getItem("passwordhotbreads")!='')
                    {
                            $('#username').val(localStorage.getItem("usernamehotbreads"));
                            $('#password_myacc').val(localStorage.getItem("passwordhotbreads"));
							user.mobile = localStorage.getItem('usernamehotbreads');
							var pwd = localStorage.getItem('passwordhotbreads');
                        ...
                        if(user.mobile=='undefined' || user.mobile=='null' || user.mobile==null)
                           ...
                           // alert(user.firstname);
                            ...
                            document.getElementById("profilepts1").innerHTML=user.total+' points';
                             ...
                             $("#wait").css("display","none");
                             authenticate();
                          if(imageExists('https://taghash.co/mloyalfileupload/uploads/hotbreads_'+localStorage.getItem("usernamehotbreads")+'.jpg'))
                          {
                              document.getElementById("cardImagemenu").src='https://taghash.co/mloyalfileupload/uploads/hotbreads_'+localStorage.getItem("usernamehotbreads")+'.jpg';
                          }                     
                     }
				   else if(localStorage.getItem("usernamehotbreads")!=null && localStorage.getItem("usernamehotbreads")!='' && (localStorage.getItem("passwordhotbreads")==null || localStorage.getItem("passwordhotbreads")=='')) 
	                 {
		 ...
		 $.ajax({
			...
			success: function(data, textStatus, xhr) {
			 ...
			 if(data.toLowerCase()=='success'){
				 ...
				 toast('Verification Code has been sent to your mobile.Please check your SMS.');
			  
				  localStorage.setItem("SMSMNO",mob);
				 
			      $('#username').val(mob);
				  localStorage.setItem('usernamehotbreads',mob);
...
function authenticate(){
  ...
  if(phno=='')
	...
	phno='';
 if(uname=='' || pwd==''){
  $('#login-msg').html('Please enter Mobile Number /Verification Code');
 }
 else{
  $.ajax({
   ...
   localStorage.setItem('usernamehotbreads',uname);
   localStorage.setItem('passwordhotbreads',pwd);
   ...
   if(user.mobile=='undefined' || user.mobile=='null' || user.mobile==null)
    ...
    {
	    if(localStorage.getItem("profiledone_hotbreads")){
    ...
    error: function(xhr, textStatus, errorThrown) {
	user.mobile = localStorage.getItem('usernamehotbreads');
	pwd = localStorage.getItem('passwordhotbreads');
	uname = user.mobile;
    user.balance = localStorage.getItem('balance');
    user.firstname = localStorage.getItem('firstname');
    user.lastname = localStorage.getItem('lastname');
...
function profileUpdate()
{
	...
	$.ajax({
			...
			success: function(data, textStatus, xhr) {
				...
				localStorage.setItem("profiledone_hotbreads","Yes");
...
$(document).on('pageinit', '#profilepage', function (event, ui) {
		...
		$(document).on('tap', '#update_button_none', function (e, ui) {
...
function showRewards(){
	...
	openInWebView('https://hotbreads.mloyalretail.com/microsite/');
	//RewardsNA();
	/*
	if(user.mobile==null || user.mobile=='')
	{
...
if(id=='rewardstore' || id=='reward-cat' || id=='reward-brand')
	{
	tabserver = "https://hotbreads.mloyalretail.com/Rewards/";
	}
else
	{
	
	tabserver = "https://hotbreads.mloyalretail.com/m/";
	}
	$.getJSON(
		tabserver+url, {
		//page: url,
...
function renderTemplate(pageID, json) {
	...
	case 'loyalty':
		...
		str += '<div class="pts"><span>' + json.balance + '</span></div><br>';   
        ...
        str += '<a href="javascript:redeemHistory();" id="ptsRedeem" class="ui-btn ui-corner-all"><img src="img/redeemed-points.png" alt="img" class="rIcon"><span>Redeemed Points :' + json.burnpoints + '</span><span class="purchaseBtnTxt">My Redemption History</span></span></a>';
		str += '<a href="javascript:bonusHistory();" id="ptsBonus" class="ui-btn ui-corner-all"><img src="img/earned-points.png" alt="img" class="rIcon"><span>Bonus Points :' + json.bonuspoints + '</span><span class="purchaseBtnTxt">Bonus Points History</span></span></a>';
		//str += '<a href="javascript:expireHistory();" id="ptsExpire" class="ui-btn ui-corner-all"><img src="img/redeemed-points.png" alt="img" class="rIcon"><span>Expired Points :' + json.expiredpoints + '</span><span class="purchaseBtnTxt">My Purchase History</span></span></a>';
	...
	case 'inbox':
        ...
        str += '<span class="msghdr">DM-BREADS</span><span class="msgTime">'+json[i].msgtime+'</span>';
...
var str = '';
	...
	switch (pageID) {
		...
		case 'aboutus':
			...
			if(user.membership_slab=='1' || user.membership_slab=='')
            {
               document.getElementById('bronzeslab').className = '';
		       ...
		       document.getElementById('bronzeslab').className = 'active';
			...
			else if(user.membership_slab=='3' || user.membership_slab=='4')
            {
               document.getElementById('bronzeslab').className = '';
               document.getElementById('silverslab').className = '';
               document.getElementById('goldslab').className = '';
  ...
  $.getJSON(
     url, {
         ...
         renderTemplate(id, json);
          ...
          localStorage.setItem('facebook_hotbreads',JSON.stringify(json.item));
		 ...
		 try{
			 renderTemplate(id,JSON.parse(localStorage.getItem('facebook_hotbreads')));
		 }catch(err)
		   {
         alert("Error: Could not connect to Server, make sure you are connected to Network");
		   }
  ...
  $.getJSON(
     url, {
         ...
         renderTemplate(id, json.feed.entry);
          ...
          localStorage.setItem('youtube_hotbreads',JSON.stringify(json.feed.entry));
		   ...
		   try{
			 renderTemplate(id,JSON.parse(localStorage.getItem('youtube_hotbreads')));
		   }
		   catch(err)
		   {
         alert("Error: Could not connect to Server, make sure you are connected to Network");
...
else if(url==RSS_twitter)
{
  ...
  $.ajax({
            ...
            localStorage.setItem('twitter_hotbreads', JSON.stringify(data));
                },
                error: function (xhr, textStatus, errorThrown) {
				...
				{
					renderTemplate(id, JSON.parse(localStorage.getItem('twitter_hotbreads')));
					//alert('here in catch');
				}
  ...
  $.getJSON(
     url, {
         ...
         renderTemplate(id, json.body);
          ...
          localStorage.setItem('pinterest_hotbreads',JSON.stringify(json.body));
		   ...
		   try{
			    renderTemplate(id,JSON.parse(localStorage.getItem('pinterest_hotbreads')));
		   }catch(err)
		   {
         alert("Error: Could not connect to Server, make sure you are connected to Network");
		   }
...
else if(url==RSS_tumblr)
{
  ...
  $.getJSON(
     url, {
        ...
        //  localStorage.setItem('twitter',JSON.stringify(json.responseData.feed.entries));
       localStorage.setItem('tumblr_hotbreads',JSON.stringify(json.channel.item));

    

       }).error(function () {
		   try{
			   renderTemplate(id,JSON.parse(localStorage.getItem('tumblr_hotbreads')));
		   }catch(err)
		   {
         alert("Error: Could not connect to Server, make sure you are connected to Network");
		   }
...
else if(url==RSS_instagram)
{
  ...
  $.getJSON(
     url, {
        ...
        //  localStorage.setItem('twitter',JSON.stringify(json.responseData.feed.entries));
       localStorage.setItem('instagram_hotbreads',JSON.stringify(json.user));

    

       }).error(function () {
		   try{
			   renderTemplate(id,JSON.parse(localStorage.getItem('instagram_hotbreads')));
		   }catch(err)
		   {
         alert("Error: Could not connect to Server, make sure you are connected to Network");
		   }
...
function receivedEvent(id) {
 ...
 var push = PushNotification.init({ "android": {"senderID": "924497757626"}});
...
function addincart(i)
{
	...
	cart.cartdata.push({ 
   "index":i,
   "url" : "https://hotbreads.mloyalretail.com/"+jsonary[i].brand_logo,
...
function checkout(){
  ...
  $.ajax({
    url: 'https://hotbreads.mloyalretail.com/Rewards/complete.asp',
    type: 'GET',
    timeout: 50000,
...
function giftfriend(){
  ...
  var val=$('#giftpt').val();
  $.ajax({
    url: 'https://hotbreads.mloyalretail.com/m/gift_points_api.asp',
    type: 'GET',
    timeout: 50000,
    dataType: 'text', //xml/html/script/json/jsonp
...
function shareApp()
{
	window.plugins.socialsharing.share('Hi, Join Hot Breads Rewards.', null, 'https://taghash.co/shareimg/hotbreads.jpg', 'https://hotbreads.mloyalretail.com/mapp');
}
		 ...
		 if (deviceType=='iPad' || deviceType=='iPhone') 
			 {
				...
				window.open('https://hotbreads.mloyalretail.com/mapp');
			 } 
		 else if (deviceType=='Android') 
			 {
               window.open("market://details?id=com.mobiquest.hotbreads","_system");
...
{
	var ref = window.open('https://www.facebook.com/hotbreadsindia', '_blank');
}

function showTwitterNew_1()
{
	var ref = window.open('https://twitter.com/hotbreadsindia', '_blank');
}

function showInstagramNew_1()
{
	var ref = window.open('https://www.instagram.com/hotbreads.co.in', '_blank');
}

function showYoutubeNew_1()
{
	var ref = window.open('https://www.youtube.com/channel/UCGGiFf17iqdXZlnW21_QGaA', '_blank');
}

function showLinkedinNew_1()
{
	var ref = window.open('https://in.linkedin.com/company/hotbreadsindia', '_blank');
}
...
function showShop()
{
    //openInWebView('https://www.hotbreads.co.in/');
	var ref = window.open('https://www.hotbreads.co.in/', '_blank');
   ...
   timeout: 50000,
    ...
    success: function(data, textStatus, xhr) {
        ...
        for (var i = 0; i < data.length; i++)
			{
				if(data[i].storename.indexOf('Email Admin')==0 || data[i].storename.indexOf('ONLINE')==0 || data[i].storename.indexOf('online')==0 || data[i].storename.indexOf('Paytm App Store')==0 || data[i].storename.indexOf('ShortCode')==0 || data[i].storename.indexOf('Mobile App')==0 || data[i].storename.indexOf('Paytm Store')==0|| data[i].storename.indexOf('MicroSite')==0 || data[i].storename.toLowerCase().indexOf('test')==0)
				continue;
...
function purchaseHistory()
{
    ...
    else
	{
		...
		//var pop3 = document.getElementById('expireHistory');
...
{
	var pop = document.getElementById('expireHistory');
	if(pop.style.display=='block')
	{
		pop.style.display = 'none';
	}
    else
	{
		...
		var pop1 = document.getElementById('purchaseHistory');
		var pop2 = document.getElementById('redeemHistory');
		var pop3 = document.getElementById('bonusHistory');
		pop1.style.display = 'none';
		pop2.style.display = 'none';
		pop3.style.display = 'none';
	}
...
$("input[type=file]").change(function(){
  ...
  // alert($("input[type=file]")[0].files[0]);
	...
	aFormData.append("mobile", localStorage.getItem("usernamehotbreads"));
    aFormData.append("brand", "hotbreads");
	...
	$.ajax(
    {
      ...
      success     : 
        function(data)
        {
         ...
         document.getElementById("cardImagemenu").src='https://taghash.co/mloyalfileupload/uploads/hotbreads_'+localStorage.getItem("usernamehotbreads")+'.jpg';
      //   document.getElementById("cardImage").src='http://103.25.128.61:9296/mloyalfileupload/uploads/hotbreads_'+localStorage.getItem("usernamehotbreads")+'.jpg';
        }
    });
...
function logoutApp()
{
	localStorage.setItem("usernamehotbreads",'');
	localStorage.setItem("passwordhotbreads",'');
	localStorage.setItem("profiledone_hotbreads",'');
...
function uploadPhoto(imageURI) {
				...
				var fName='hotbreads_'+user.mobile;
...
function getBalance()
{
	...
	$.ajax({
					...
					success: function(data, textStatus, xhr) {
						  ...
						  document.getElementById("tpoint").innerHTML = 'POINTS: ' +data.balance;
						  document.getElementById("profilepts").innerHTML = data.balance+' Points';
						  document.getElementById("profilepts1").innerHTML = data.balance+' Points';
